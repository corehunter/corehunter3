...
public class SimpleDistanceMatrixDataTest extends TestData {
    ...
    public void loadFromFileTest1() {
        try {
            testData(SimpleDistanceMatrixData.readData(new FileProperties(
                    SimpleDistanceMatrixDataTest.class.getResource(TXT_FILE1).getFile()
            )));
        } catch (IOException e) {
            e.printStackTrace();
            fail(e.getMessage());
    ...
    public void loadFromFileTest2() {
        try {
            testData(SimpleDistanceMatrixData.readData(new FileProperties(
                    SimpleDistanceMatrixDataTest.class.getResource(TXT_FILE2).getFile(), true, true
            )));
        } catch (IOException e) {
            e.printStackTrace();
            fail(e.getMessage());
